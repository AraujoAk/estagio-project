...
class ContactController {
    ...
    def status(Integer id) {
        String status = contactService.getStatus(id)
        render(status)
    }
    ...
    def delete(Integer id) {
        ...
        if (!response) {
            ...
            if (response.image) {
                def uploadDir = grailsApplication.config.upload.directory ?: "web-app/uploads/"
                def imagePath = uploadDir + response.image
